...
fun main(args: Array<String>) {
    ...
    prepareJarAndRun(KOTLIN_HOME, kotlinRunner, executor, scriptResource, docopt,
...
fun prepareJarAndRun(KOTLIN_HOME: String,
    ...
    if (docopt != null && docopt.getBoolean("add-bootstrap-header")) {
        errorIf(!rawScriptFile.canWrite()) {
            "Script file not writable: $rawScriptFile"
        }
        errorIf(rawScriptFile.parentFile == SCRIPT_TEMP_DIR) {
            "Temporary script file detected: $rawScriptFile, created from $scriptResource"
        }
        val scriptLines = rawScriptFile.readLines().dropWhile {
        ...
        rawScriptFile.writeText((BOOTSTRAP_HEADER + scriptLines).joinToString("\n"))

        if (loggingEnabled) {
            info("$rawScriptFile updated")
        }
        quit(0)
    }
    ...
    docopt?.let {
        if (docopt.getBoolean("idea")) {
            ...
            executor.execute(launchIdeaWithKscriptlet(rawScriptFile, script.dependencies, script.repos, includeURLs))
            exitProcess(0)
        }
    }
    ...
    script.compiles.forEach {
        ...
        currentClasspath = prepareJarAndRun(KOTLIN_HOME, kotlinRunner, executor, file, docopt = null,
